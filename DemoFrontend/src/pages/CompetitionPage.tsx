...
import { CandidateSite, CompetitorPoint } from '../types';
import { DISTANCE_DECAY_DATA } from '../data/suratData';
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-rose-50 text-rose-700 border border-rose-100">
              Spatial Density Index
            </span>
          </div>
          <p className="text-xs text-slate-500 mt-1">
            Analyzing spatial competitor distribution and cannibalization risk around <strong className="text-slate-800">{currentSite.name}</strong>.
          </p>
        </div>

        <div className="flex items-center gap-3">
          <span className="text-xs font-semibold text-slate-500">Overall Competition Density:</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-slate-900">
                Competitor Spatial Scatter (Surat)
              </h3>
            </div>
            <div className="flex items-center gap-2 text-xs">
              <span className="w-2.5 h-2.5 rounded-full bg-rose-500" />
              <span className="text-slate-500">Competitor Pin</span>
            </div>
          </div>
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-xs space-y-4">
              ...
              {DISTANCE_DECAY_DATA.map((item) => (
                <div key={item.distance} className="space-y-1">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-semibold text-slate-700">{item.distance}</span>
                    <div className="flex items-center gap-2 font-mono">
                      <span className="text-slate-400">{item.competitors} sites</span>
                      <span className="font-bold text-rose-600">{item.impact}% impact</span>
                    </div>
                  </div>
            ...
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 text-xs text-slate-600 leading-relaxed flex gap-2.5 items-start">
              ...
              {competitors.slice(0, 3).map((comp) => (
                ...
                >
                  <div className="min-w-0">
                    <span className="font-bold text-slate-800 block truncate">{comp.name}</span>
                    <span className="text-[11px] text-slate-400">{comp.brand}</span>
                  </div>
                  <div className="text-right shrink-0">
                    <span className="font-bold text-slate-800 block">{comp.distanceKm} km</span>
                    <span className="text-[10px] text-indigo-600 font-semibold">{comp.category}</span>
                  </div>
                </div>
              ))}
            </div>
